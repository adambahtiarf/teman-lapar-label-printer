...
export async function getAttributesClient(activeOnly = false) {
...
export async function getMenusClient(activeOnly = false) {
  const supabase = createClient()
  let query = supabase
    .from("menus")
    .select(
...
export async function getOrderClient(id: string) {
  const supabase = createClient()
...
export async function getOrderItemForPrintClient(itemId: string) {
  const supabase = createClient()
  const { data, error } = await supabase
    .from("order_items")
    .select(
...
export async function getTodaySummaryClient() {
  const supabase = createClient()
